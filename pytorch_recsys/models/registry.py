...
class ModelMetadata(BaseModel):
    ...
    model_class: Type[BaseSequentialModel] = Field(..., description="Model class reference")
    tags: List[str] = Field(default_factory=list, description="Model tags for categorization")
    requirements: Dict[str, str] = Field(default_factory=dict, description="Minimum requirements")
...
class ModelRegistry:
    ...
    def register(
        ...
        Example:
            ...
            ```
        """
        def decorator(model_class: Type[BaseSequentialModel]) -> Type[BaseSequentialModel]:
            cls._register_model(
                name=name,
    ...
    def _register_model(
        ...
        if name in cls._models:
            ...
            logger.warning(f"Model '{name}' is already registered (version {existing_version}). "
        ...
        if not issubclass(model_class, BaseSequentialModel):
            raise TypeError(f"Model {model_class.__name__} must inherit from BaseSequentialModel")
        ...
        logger.info(f"Registered model '{name}' (class: {model_class.__name__}, version: {version})")
...
def create_model(name: str, config: Union[DictConfig, Dict[str, Any]]) -> BaseSequentialModel:
