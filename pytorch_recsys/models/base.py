...
class ModelConfig(BaseModel):
    ...
    num_users: Optional[int] = Field(None, gt=0, description="Number of users (optional)")
    ...
    weight_decay: float = Field(0.0, ge=0, le=1.0, description="L2 regularization weight")
    
    # Optional scheduler configuration
    scheduler: Optional[Dict[str, Any]] = Field(None, description="Learning rate scheduler config")
    
    # Loss function parameters
    loss_type: str = Field("cross_entropy", description="Loss function type")
    label_smoothing: float = Field(0.0, ge=0, le=1.0, description="Label smoothing factor")
    ...
    def validate_embedding_dim(cls, v):
        if v % 8 != 0:
            raise ValueError("embedding_dim should be divisible by 8 for optimal performance")
        return v
...
class BaseSequentialModel(pl.LightningModule, ABC):
    ...
    def predict(self, sequences: Tensor, k: int = 10, return_scores: bool = False) -> Union[Tensor, Tuple[Tensor, Tensor]]:
    ...
    @classmethod
    def load_model(cls, path: str, config: Optional[DictConfig] = None) -> 'BaseSequentialModel':
